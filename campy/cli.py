...
def ParseClargs(parser):
	...
	parser.add_argument(
		...
		dest="qualityMode",
		type=str,
		help="ffmpeg quality mode following '-rc' (e.g., 'cbr', 'constqp', 'vbr'). \
		See ffmpeg -h encoder=<codec> for help with modes valid for specific video codecs.",
	)
	parser.add_argument(
		"--preset",
		dest="preset",
	...
	parser.add_argument(
		"--displayFrameCounter",
		dest="displayFrameCounter",
		type=float,
		help="Period to display fps and recording duration stats.",
	)
	parser.add_argument(
		"--displayFrameRate",
